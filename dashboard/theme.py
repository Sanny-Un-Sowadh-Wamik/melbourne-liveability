...
def inject(grad=("#0b6e4f", "#0f9d58", "#0891b2"), accent="#0f9d58") -> None:
    st.markdown(
        f"""
        <style>
          ...
          .hero {{
            background: linear-gradient(120deg, {grad[0]}, {grad[1]}, {grad[2]});
            background-size: 220% 220%; animation: grad 14s ease infinite;
            border-radius: 18px; padding: 26px 30px; color: #fff; margin-bottom: 16px;
            box-shadow: 0 12px 34px rgba(10, 30, 25, 0.22);
          }}
          ...
          .badge {{display:inline-block; padding:3px 11px; border-radius:999px; font-size:.74rem;
                   font-weight:700; letter-spacing:.3px; margin-bottom:10px;
                   background:rgba(255,255,255,.18); border:1px solid rgba(255,255,255,.3);}}

          div[data-testid="stMetric"] {{
            background:#fff; border:1px solid #eef0f3; border-radius:14px; padding:14px 16px;
          ...
          .livedot {{height:9px;width:9px;background:#22c55e;border-radius:50%;display:inline-block;margin-right:7px;
